...
namespace DictionaryWithHashTable
{
    ...
    public interface IDictionary<K, V> : IEnumerable<KeyValuePair<K, V>>
    {
        ...
        /// <summary>Assigns the specified value to the specified 
        /// key in the dictionary. If the key already exists, its value 
        /// is replaced with the new value and the old value is returned
        /// </summary>
        /// <param name="key">Key for the new value</param>
        /// <param name="value">Value to be mapped to that key</param>
        /// <returns>the old (replaced) value for the specified key or 
        /// null if the key does not exist</returns>
        V Set(K key, V value);
