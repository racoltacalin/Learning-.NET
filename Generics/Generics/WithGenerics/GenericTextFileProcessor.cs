...
namespace Generics.WithGenerics
{
    public static class GenericTextFileProcessor
    {
        public static List<T> LoadFromTextFile<T>(string filePath) where T : class, new()
        {
            ...
            // Checks to be sure we have at least one header row and onde data row
            if (lines.Count < 2)
            {
            ...
            foreach (var row in lines)
            {
                ...
                // Splites the row into individual columns. Now the index
                ...
                // the matchin column, we can do the "SetValue" method to 
        ...
        public static void SaveToTextFile<T>(List<T> data, string filePath) where T : class, new()
        {
            ...
            {
                throw new ArgumentNullException("data", "You must pupulate the data parameter with at least one element");
            }
